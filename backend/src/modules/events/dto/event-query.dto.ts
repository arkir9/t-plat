...
import { IsOptional, IsString, IsEnum, IsDateString, IsBoolean, IsNumberString, IsArray } from 'class-validator';
...
export class EventQueryDto extends PaginationDto {
    ...
    @ApiPropertyOptional({ description: 'Filter events starting on or before this date (alias for startDateTo)' })
    @IsOptional()
    @IsDateString()
    endDate?: string;
